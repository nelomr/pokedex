...
- Add `PokemonDetailSkeleton.vue`: a content-shaped placeholder rendered while the detail request is in flight, mirroring the loaded layout's blocks (artwork, title, type badges, stat rows) so the modal does not reflow when real data arrives. Announced politely to assistive technology via `aria-busy`, with animation suppressed under `prefers-reduced-motion`.
...
### New Capabilities
- `pokemon-detail`: viewing a Pokémon's details (types, base stats, abilities, official artwork) in an accessible modal, opened from a catalog card click, with a content-shaped skeleton placeholder while the detail data loads.
